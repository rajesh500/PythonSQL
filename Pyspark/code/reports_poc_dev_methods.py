...
def detail_tab_func(rdf_cw_dtf, rdf_li_dtf, rdf_tf_dtf, report_data_dtf):
    writer = pd.ExcelWriter(file_path, engine='xlsxwriter')
    rdf_cw_dtf.to_excel(writer, sheet_name='Details', index=False, startrow=7)
    rdf_li_dtf.to_excel(writer, sheet_name='Details', index=False, startrow=17)
    rdf_tf_dtf.to_excel(writer, sheet_name='Details', index=False, startrow=27)
    wbsb = writer.book
    ...
    county_name = report_data_dtf.county.to_string(index = False)
    startdate = report_data_dtf.start_date__c.to_string(index = False)
    enddate = report_data_dtf.end_date__c.to_string(index = False)
    ...
    legends_tab_func(writer)
    writer.close()



''' Legends tab and styling the page '''
def legends_tab_func(writer):
